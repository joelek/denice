...
filter_kernel(__global float* buffer, __read_only image2d_t source, int x, int y, float threshold, float weight) {
	...
	buffer[(coords.y * ss.x) + coords.x] += t * weight;
}
