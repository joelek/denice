...
-> void {
	...
	OPENCL_CHECK_STATUS();
	if (pass_is_even) {
		status = filter_kernel.setArg(1, data_channel_prev.source);
		OPENCL_CHECK_STATUS();
		status = filter_kernel.setArg(2, data_channel.source);
		OPENCL_CHECK_STATUS();
		status = filter_kernel.setArg(3, data_channel_next.source);
		OPENCL_CHECK_STATUS();
	} else {
		status = filter_kernel.setArg(1, data_channel_prev.target);
		OPENCL_CHECK_STATUS();
		status = filter_kernel.setArg(2, data_channel.target);
		OPENCL_CHECK_STATUS();
		status = filter_kernel.setArg(3, data_channel_next.target);
		OPENCL_CHECK_STATUS();
	}
...
-> int {
	try {
		...
		auto filter_kernel = get_opencl_kernel(program, "filter_kernelv1");
		auto normalize_kernel = get_opencl_kernel(program, "normalize_kernel");
		auto postfilter_kernel = get_opencl_kernel(program, "filter_kernelv1");
		...
		while (!feof(stdin) || (frames_written < frames_read)) {
			for (auto i = frames_read; i < frames_written + frame_buffer_capacity; i++) {
				...
				if (arg_strength > 0.0) {
					...
					prefilter_frame(queue, lowpass_x_kernel, lowpass_y_kernel, frame, 0);
				}
